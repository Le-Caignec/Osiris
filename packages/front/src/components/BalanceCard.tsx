...
const BalanceCard: React.FC = () => {
  ...
  return (
    <div className='bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl p-4 border border-gray-700 shadow-xl hover:shadow-2xl transition-all duration-300'>
      <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 gap-2 sm:gap-0'>
        ...
        <div
          className={`px-2 py-1 rounded-full bg-gradient-to-r ${getNetworkColor()} text-white text-xs font-medium self-start sm:self-auto`}
        >
          {getNetworkName()}
        </div>
      </div>
        ...
        {balanceItems.map((item, index) => (
          ...
          >
            <div className='flex items-center space-x-3 min-w-0 flex-1'>
              ...
              <span className='text-gray-300 font-medium text-base truncate'>
                {item.label}
              </span>
            </div>
            <span
              className={`font-bold text-lg ${item.color} flex-shrink-0 ml-3`}
            >
              {item.value}
            </span>
          </div>
